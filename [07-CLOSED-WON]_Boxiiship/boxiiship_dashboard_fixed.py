...
with tab2:
    ...
    with col1:
        ...
        fig1.update_layout(
            ...
            yaxis=dict(
                ...
                title_font=dict(color=COLORS['primary_blue']),
                tickfont=dict(color=COLORS['primary_blue'])
            ),
            yaxis2=dict(
                title='Average Transit Days',
                title_font=dict(color=COLORS['primary_orange']),
    ...
    with col2:
        ...
        bins = list(range(0, int(delivered_df['Days In Transit'].max()) + 2))
        hist_data = pd.cut(delivered_df['Days In Transit'], bins=bins, include_lowest=True)
        hist_counts = hist_data.value_counts().sort_index()
        
        fig2 = go.Figure()
        
        # Add bars for each transit day
        for i, (interval, count) in enumerate(hist_counts.items()):
            color = COLORS['green'] if i < 3 else COLORS['primary_orange'] if i < 5 else COLORS['red']
        ...
        fig2.update_layout(
            ...
            xaxis=dict(title='Transit Days'),
            yaxis=dict(title='Number of Packages'),
            showlegend=False
        )
