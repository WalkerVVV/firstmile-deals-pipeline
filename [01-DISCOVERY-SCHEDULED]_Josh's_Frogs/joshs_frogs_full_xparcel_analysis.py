#!/usr/bin/env python3
"""
Josh's Frogs - Complete Xparcel Analysis
...
# Initialize ZIP search engine for zone calculation
print("Initializing ZIP code database for zone calculation...")
search = SearchEngine()

def get_zone_from_zips(origin_zip, dest_zip):
    ...
    try:
        ...
        origin_data = search.by_zipcode(origin_zip_str)
        dest_data = search.by_zipcode(dest_zip_str)

        if origin_data and dest_data and origin_data.lat and dest_data.lat:
            # Calculate distance in miles
            from math import radians, cos, sin, asin, sqrt
            lon1, lat1 = radians(origin_data.lng), radians(origin_data.lat)
            lon2, lat2 = radians(dest_data.lng), radians(dest_data.lat)
            ...
            # Convert miles to zones (approximate FirstMile zone mapping)
            if miles < 50:
                return 1
            elif miles < 150:
                return 2
...
df['Zone'] = df.apply(lambda row: get_zone_from_zips(row['Origin'], row['Destination']), axis=1)
