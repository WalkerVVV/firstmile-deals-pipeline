...
print("=" * 70)
print("BoxiiShip Complete In-Transit Tracking Parser")
print("=" * 70)

# Read the raw tracking data
    ...
    if line and line[0].isdigit() and len(line) >= 18:
        ...
        if i+2 < len(lines) and lines[i+2]:
            ...
            if any(day in potential_date for day in days):
                ...
                if i+3 < len(lines) and lines[i+3] and lines[i+3][0].isupper():
                    location = lines[i+3]
...
print(f"\n[SUCCESS] Parsed {len(df)} total tracking numbers")
...
output_file = 'In_Transit_MP_Tracking_Complete_Nov4_2025.xlsx'

with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
    df.to_excel(writer, index=False, sheet_name='All Tracking Data')

    # Get the worksheet
    worksheet = writer.sheets['All Tracking Data']
